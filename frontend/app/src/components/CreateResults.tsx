  ...
  return (
    <div className="pf-create-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 16, alignItems: 'start' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ background: verdict.bg, border: `1px solid ${verdict.border}`, borderRadius: 'var(--radius-md)', padding: '18px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
          <span style={{ flex: 'none', width: 38, height: 38, borderRadius: 'var(--radius-md)', background: verdict.iconBg, color: verdict.iconColor, display: 'grid', placeItems: 'center' }}>{verdict.glyph}</span>
          <div style={{ flex: 1 }}>
          ...
          <div style={{ background: 'var(--bg-2)', border: '1px solid var(--line-1)', borderRadius: 'var(--radius-md)', overflow: 'hidden' }}>
            ...
            {issues.map((e) => (
              <div key={e.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 13, padding: '14px 18px', borderBottom: '1px solid var(--line-1)' }}>
                <span style={{ flex: 'none', width: 24, height: 24, borderRadius: 'var(--radius-input)', background: e.iconBg, color: e.iconColor, display: 'grid', placeItems: 'center', marginTop: 1 }}>{e.glyph}</span>
                <div style={{ flex: 1 }}>
