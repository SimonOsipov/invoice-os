          ...
          {form.lineItems.map((row, i) => (
            <div key={i} style={{ display: 'grid', gridTemplateColumns: '140px 1fr 90px 120px 40px', gap: 10, padding: '9px 12px', borderBottom: '1px solid var(--line-1)', alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ width: 30, height: 30, borderRadius: 'var(--radius-md)', border: '1px solid var(--line-2)', background: 'var(--bg-2)', color: 'var(--fg-2)', cursor: 'pointer', display: 'grid', placeItems: 'center' }}
              >
                {closeGlyph}
              </button>
            </div>
