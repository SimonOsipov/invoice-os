...
| C18 | **Withdrawal** — what a visitor can actually do today. | CODE + honest gap | `consent.ts:8` `CONSENT_DEFAULT_ANALYTICS = false`; `consent.ts:66` `writeConsent` has **zero production call sites** (only `consent.ts` and `consent.test.ts` mention it). Expanded mechanism by mechanism in Table 2. |
...
There is no consent control on this site yet. `CONSENT_DEFAULT_ANALYTICS` is `false`, so a
visitor with no stored record is treated as declining, and `writeConsent` is never called
by anything that ships. The tag therefore does not load and no `_ga` cookie is set — and
until LAND-05 mounts the notice there is no way for a visitor to allow it either.
