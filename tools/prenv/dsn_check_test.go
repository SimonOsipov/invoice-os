...
func healthyMap() dsnMap {
	...
	// The five DOCUMENT_* rows are Required for both services that call
	// document.ConfigFromEnv at boot, so a fleet without them is not healthy.
	// Absence is asserted on explicitly in TestCheckDSNs_DocumentVarsAreRequired,
	// which deletes them again.
