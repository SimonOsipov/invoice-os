...
// The behavioural partition (TestReconcile_TheDoubtCoversExactlyThreeOfTheTenHeaderFields)
...
func TestReconcile_TheDoubtfulFieldsAreThreeRealHeaderFields(t *testing.T) {
	...
	want := []string{"buyer_name", "buyer_tin", "vat"}
	got := slices.Clone(doubtfulFields)
	slices.Sort(got)
	if !slices.Equal(got, want) {
		t.Errorf("doubtfulFields = %q, want the three %q", doubtfulFields, want)
	}
