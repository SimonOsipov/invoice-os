...
func TestDeletePolicy_NeedsAnActiveAdmin(t *testing.T) {
	...
	for _, tc := range callers {
		t.Run(tc.name, func(t *testing.T) {
			...
			rec.reset()
			_, err := NewStore(traced, stubFingerprinter, nil).DeletePolicy(c, policyID)
			if !errors.Is(err, tc.want) {
				t.Errorf("DeletePolicy as %s: err = %v, want %v", tc.name, err, tc.want)
			}
			if tc.bySeam {
...
func TestDeletePolicy_StampsThePolicyRowBeforeTheVersionWrite(t *testing.T) {
	...
	traced, rec := tracedAppPool(t)
	rec.reset()
	if _, err := NewStore(traced, stubFingerprinter, nil).DeletePolicy(c, policyID); err != nil {
