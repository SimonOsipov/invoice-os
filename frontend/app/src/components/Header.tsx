  ...
  return (
    <header style={{ flex: 'none', height: 56, borderBottom: '1px solid var(--line-1)', background: 'oklch(98.5% .008 85 / .82)', backdropFilter: 'blur(12px)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
        <div className="pf-header-search" style={{ display: 'flex', alignItems: 'center', gap: 8, height: 34, padding: '0 12px', border: '1px solid var(--line-2)', borderRadius: 'var(--radius-md)', background: 'var(--bg-2)', width: 240 }}>
        ...
        <button
          ...
          className="pf-btn"
          style={{ display: 'flex', alignItems: 'center', gap: 7, height: 34, padding: '0 11px', borderRadius: 'var(--radius-md)', border: `1px solid ${sandbox ? 'var(--status-amber-border)' : 'var(--line-2)'}`, background: sandbox ? 'var(--status-amber-bg)' : 'transparent', color: sandbox ? 'var(--status-amber-text)' : 'var(--fg-3)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 10, fontWeight: 600, letterSpacing: '0.06em' }}
        >
