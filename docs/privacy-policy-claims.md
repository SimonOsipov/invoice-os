...
| C14 | The exact sentence you ticked is stored alongside your details. | CODE | `demoForm.ts:42-43` `CONSENT_TEXT`, one constant read by both the checkbox label (`DemoLeadForm.tsx`, the shared form both the popup and the landing card mount) and the payload (`hubspot.ts:104`). The privacy page quotes the same constant rather than retyping it, so all three can never disagree. |
