...
export type View = 'dashboard' | 'invoices' | 'rules' | 'workflows' | 'create' | 'detail' | 'clients' | 'customers' | 'reports' | 'settings' | 'approvals' | 'audit' | 'extraction'
...
export type PlatformCtx = {
  ...
  // Drops the active group's restored snapshot; no undo.
  ...
  restartImport: () => void
  skipUpload: () => void
  // The route out of a document that produced no invoice. Reads the document's carried reading,
  // then lands on the 'form' step; the filing carries the document either way.
