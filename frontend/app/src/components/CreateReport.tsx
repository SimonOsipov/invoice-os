...
export function CreateReport({ ctx }: { ctx: PlatformCtx }) {
  ...
  if (summary.kind === 'failed') {
    return (
      <div style={{ background: 'var(--bg-2)', border: '1px solid var(--status-red-border)', borderRadius: 'var(--radius-lg)', padding: '24px 22px', maxWidth: 720 }}>
            ...
            return (
              ...
              >
                <div style={{ flex: 'none', width: 110 }}>
                  <div className="mono" style={{ fontSize: 11.5, fontWeight: 600, color: clickable ? 'var(--action)' : 'var(--fg-2)' }}>{v.invoice_number}</div>
