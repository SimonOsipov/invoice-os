...
func TestServiceImport_DryRunClassifiesOnlyDBUnchanged(t *testing.T) {
	...
	res, err := svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, mixedFileFixture(), true)
	if err != nil {
...
func TestServiceImport_SameMixedFileRealImportMatchesDryRunVerdict(t *testing.T) {
	...
	res, err := svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, mixedFileFixture(), false)
	if err != nil {
...
func TestServiceImport_MappingMissingInvoiceNumberValidationBeforeAnyWrite(t *testing.T) {
	...
	res, err := svc.Import(c, entityID, "", "", 1, mapping, stdHeader, rows, false)
...
func TestServiceImport_ConcurrentDuplicateAtCreateTimeQuarantinesLoser(t *testing.T) {
	...
	for i := 0; i < racers; i++ {
		go func(i int) {
			...
			results[i], errs[i] = svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, raceRow, false)
		}(i)
	}
	go func() {
		defer wg.Done()
		...
		results[racers], errs[racers] = svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, cleanRow, false)
	}()
	close(start)
	wg.Wait()
...
func TestServiceImport_EntityScopedDedupSameNumberUnderDifferentEntityNotDuplicate(t *testing.T) {
	...
	res, err := svc.Import(c, entityA, "", "", 1, stdMapping, stdHeader, rows, false)
	if err != nil {
		t.Fatalf("Import: %v", err)
	}
	if res.ReadyInvoices != 1 || res.QuarantinedInvoices != 0 || len(res.Errors) != 0 {
...
func TestServiceImport_DryRunExactVerdictForNonNumericField(t *testing.T) {
	...
	dryRes, err := svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, nonNumericTotalFixture(), true)
	if err != nil {
		t.Fatalf("Import (dry-run): %v", err)
	}
	...
	realRes, err := svc.Import(c, entityID, "", "", 1, stdMapping, stdHeader, nonNumericTotalFixture(), false)
	if err != nil {
		t.Fatalf("Import (real): %v", err)
	}
