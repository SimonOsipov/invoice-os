...
// fixture except the two Chrome ones (28 files): the loop is identical and covers 28 for free.
func TestCharsIn_ReproducesEveryRectTextOnTheCorpus(t *testing.T) {
	names := pdcNonChromeFixtures(t)
	if len(names) != 28 {
		t.Fatalf("pdcNonChromeFixtures returned %d name(s), want exactly 28 (30 committed minus %s and %s)", len(names), chrRegister, chrRegisterTwin)
	}
...
// token per rect" only holds on the 28 word-level fixtures now. chrome_register.pdf's 802 rects
