...
export function XmlModal({ ctx }: { ctx: PlatformCtx }) {
  ...
  return (
    <div onClick={ctx.closeXml} style={{ position: 'fixed', inset: 0, zIndex: 80, background: 'oklch(16% .03 210 / .42)', backdropFilter: 'blur(2px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, animation: 'popIn 140ms ease-out' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: 760, maxWidth: '100%', maxHeight: '100%', background: 'var(--bg-2)', border: '1px solid var(--line-2)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--shadow-elegant)', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
