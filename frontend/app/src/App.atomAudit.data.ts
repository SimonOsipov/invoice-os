...
export const AUDITED_ATOMS: readonly AuditedAtom[] = [
  ...
  {
    binding: 'handOffReading, setHandOffReading',
    name: 'handOffReading',
    kind: 'useState',
    resetBySwitchClient: true,
    routes: ['/create'],
    verdict: 'correctly-reset',
    citation: { in: 'function switchClient(id: string)', text: 'setHandOffReading(null)' },
    note: 'Cleared beside handOffDocumentId at App.tsx#switchClient, App.tsx#openCreate and App.tsx#skipUpload. Read by CreateForm at createStep `form` and by App.tsx#fileDraft.',
  },
  {
    binding: 'handOffReadSeq',
    name: 'handOffReadSeq',
    kind: 'useRef',
    resetBySwitchClient: true,
    routes: [],
    verdict: 'correctly-reset',
    citation: { in: 'function switchClient(id: string)', text: 'handOffReadSeq.current += 1' },
    note: 'Not on ctx. Bumped by App.tsx#enterByHand and by every clear of handOffReading, so a read pending across App.tsx#switchClient lands nothing.',
  },
  {
  ...
  },
  {
    ...
    verdict: 'correctly-reset',
    citation: { in: 'function switchClient(id: string)', text: 'resetImport(id)' },
    note: "Read by CreateUpload ('upload') and CreateMapping ('mapping') (ctx.importError), neither reachable after a switch regardless. Cleared by App.tsx#switchClient's resetImport(id) call (ROUTE-06-03).",
  },
  {
  ...
  },
  {
    ...
    verdict: 'deliberate',
    citation: { in: 'function switchClient(id: string)', text: 'resetImport(id)' },
    note: 'Not on ctx. Names the operation holding reqInFlight and the runSeq it started under. A switch leaves it set because that operation is still in flight; the runSeq bump in App.tsx#resetImport marks it as left.',
  },
]
