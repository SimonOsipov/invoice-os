...
var t1OwningPhraseIDs = []string{"party_ref", "signature", "reg_identifier", "rc_number", "doc_title", "due_date", "withholding_tax"}
...
func TestTier1_ReusesTheAnchorLexiconPatterns(t *testing.T) {
	if len(anchorLexicon) != 18 {
		t.Fatalf("anchorLexicon holds %d entry/entries, want 18; the coverage assertion below would run over the wrong table", len(anchorLexicon))
	}
	if len(Tier1Rules) != tier1RuleCount {
		t.Fatalf("Tier1Rules holds %d rule(s), want %d; every assertion below would run over the wrong set", len(Tier1Rules), tier1RuleCount)
	}
