...
func TestGetBatch_ReturnsFrozenCountsAndErrors(t *testing.T) {
	...
	batchID, err := store.CreateBatch(c, entityID, "", "", 0)
	if err != nil {
...
func TestGetBatch_ZeroInvoicesVersionIsNullAndBodyRendersNull(t *testing.T) {
	...
	stampedBatch, err := store.CreateBatch(c, entityID, "", "", 0)
	if err != nil {
	...
	emptyBatch, err := store.CreateBatch(c, entityID, "", "", 0)
	if err != nil {
...
func TestRLS_GetBatchCrossTenantIs404AndIdenticalToUnknown(t *testing.T) {
	...
	batch1, err := store.CreateBatch(c1, entity1, "", "", 0)
	if err != nil {
	...
	batch2, err := store.CreateBatch(c2, entity2, "", "", 0)
	if err != nil {
