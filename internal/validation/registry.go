...
// assembles the production Engine by registering all ten rule-type
// Evaluators (the five presence/shape evaluators from evaluators.go, the
// four arithmetic/aggregate evaluators from evaluators_math.go, and the
// CEL escape hatch from cel.go) against their RuleType keys, and injects
// celGuard as the select-stage `when`-clause GuardFunc.
//
// The ten evaluator structs are deliberately unexported (unit-tested
...
// NewDefaultEngine builds the production engine: all ten rule-type
