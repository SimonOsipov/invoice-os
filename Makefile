...
.DEFAULT_GOAL := help
.PHONY: help db-bootstrap dev-db dev-db-down dev-db-reset migrate-up migrate-down migrate-reset migrate-status migrate-create test-rls test-queue test-audit test-reconciliation test-approvals
...
test-approvals: ## Run the workflow-roles + staffing suite against the local dev DB (run `make dev-db` first)
	...
	DATABASE_MIGRATION_URL="$(DEV_DB_MIGRATION_URL)" \
	DATABASE_SUPERUSER_URL="$(DEV_DB_SUPERUSER_URL)" \
	go test -p 1 -count=1 ./internal/approval/...
