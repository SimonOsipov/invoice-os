...
import { GEAR_ICON, GLOBE_ICON, NAV_ITEMS } from '../data'
...
export function Sidebar({ screen, onNavigate, deadLetterCount }: Props) {
  return (
    <aside
      ...
      style={{ width: 240, flex: 'none', background: 'var(--bg-2)', borderRight: '1px solid var(--line-1)', display: 'flex', flexDirection: 'column' }}
    >
      <div style={{ padding: '16px 16px 14px', borderBottom: '1px solid var(--line-1)' }}>
        ...
        >
          <span style={{ flex: 'none', width: 26, height: 26, borderRadius: 5, background: 'var(--accent)', color: '#fff', display: 'grid', placeItems: 'center' }}>{GLOBE_ICON}</span>
          <span style={{ flex: 1, minWidth: 0 }}>
            <span style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--accent-soft)' }}>All tenants</span>
      ...
      <nav style={{ flex: 1, padding: '12px 10px', display: 'flex', flexDirection: 'column', gap: 2 }}>
        ...
        {NAV_ITEMS.map((n) => {
          ...
          const badgeBg = n.key === 'submissions' ? 'var(--status-red-bg)' : 'var(--accent-tint)'
          const badgeColor = n.key === 'submissions' ? 'var(--status-red-text)' : 'var(--accent)'
          return (
            <button key={n.key} type="button" onClick={() => onNavigate(n.key)} className="ops-nav" style={navBtnStyle(active)}>
              ...
              {badge && (
                <span
                  className="mono ops-nav-label"
                  style={{ fontSize: 10, fontWeight: 700, background: badgeBg, color: badgeColor, borderRadius: 99, padding: '1px 6px' }}
                >
                  {badge}
                </span>
              )}
