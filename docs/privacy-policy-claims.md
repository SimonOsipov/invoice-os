...
to defend a sentence on `https://www.ascomply.com/privacy` to a member of the public.
...
## Table 1 — the page's factual claims (C1–C19)
...
| C15 | HubSpot holds it on their EU servers. | CODE | `hubspot.ts:44-46` — `https://api-eu1.hsforms.com/...`, the EU-region Forms host. |
...
| C19 | Contact address for a data request: `sam@ascomply.com`. | **OPERATOR-CONFIRMED** | User supplied the address on 2026-08-16 at the planning fork gate. No address was published on the site before this page. **The rendered page must never contain `e.iroha@ascomply.com`** — that is a mock sign-in persona (`frontend/landing/src/auth.ts:53`) whose inbox nobody reads, and it is one keystroke away from a real address. A test pins its absence permanently. |
