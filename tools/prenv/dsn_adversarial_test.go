...
// bidirectional: it fails on a bad table row AND on a bad filter change. It is
// also what keeps this a guard rather than a decoration once the filter carries
// more than one prefix -- a Go-side list would be satisfied by itself.
//
// KILLS: M-prefix, and any narrowing of the prefix list.
func TestEverySeverityTableRowSurvivesTheShellPrefixFilter(t *testing.T) {
	prefixes := shellDSNVarPrefixes(t)
...
// dsnVarPrefixAssignment matches the prefix declaration in railway-env.sh. The
// name is allowed to grow a suffix (DSN_VAR_PREFIXES, DSN_VAR_PREFIX_RE)
// because the bash spelling of a list is the executor's call; what is not
// negotiable is that exactly one line declares it.
var dsnVarPrefixAssignment = regexp.MustCompile(`(?m)^(DSN_VAR_PREFIX[A-Z_]*)=(.*)$`)

// shellDSNVarPrefixes reads the prefix list out of railway-env.sh, tolerating
// the plausible bash spellings: a bare string, a quoted space-separated list, a
// `( ... )` array, or a `A|B` alternation.
func shellDSNVarPrefixes(t *testing.T) []string {
	t.Helper()
	...
	matches := dsnVarPrefixAssignment.FindAllStringSubmatch(string(content), -1)
	if len(matches) == 0 {
		t.Fatalf("could not find a DSN_VAR_PREFIX* assignment in %s. If the prefix filter was removed, delete this test deliberately; if it was renamed, update this pattern -- do not let the coupling go unasserted.", path)
	}
	if len(matches) > 1 {
		var names []string
		...
		t.Fatalf("%s declares the prefix filter in %d places (%v). Two sources of truth means one of them is unused and drifts silently.", path, len(matches), names)
	}

	rhs := matches[0][2]
	// Strip quoting, array parens and regex anchors, then split on whitespace
	// and the alternation/list separators.
	...
	// Non-vacuity: an empty or absent prefix makes startswith() universally
	// true, so the caller's loop would pass against a filter that filters
	// nothing.
	if len(prefixes) == 0 {
		t.Fatalf("the prefix assignment %q parsed to no prefixes -- every check would pass vacuously", matches[0][0])
	}
	for _, p := range prefixes {
		if p == "" {
			t.Fatalf("the prefix assignment %q yields an empty prefix -- every check would pass vacuously", matches[0][0])
		}
	}
	return prefixes
}
